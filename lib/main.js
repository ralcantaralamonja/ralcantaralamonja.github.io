...
fetch("https://webservice-nrc1042.000webhostapp.com/canciones_sw.php")
    ...
    function llenarTablaEnvios(data){
        ...
        data.map( item => {
            ...
            fila += "<td>" + item.NombreCancion + "</td>";
            fila += "<td>" + item.FechaLanzamiento + "</td>";
            fila += "<td>" + item.Duracion + "</td>";
            fila += "<td>" + item.Albun + "</td>";
            fila += "</tr>";
            contenidoTabla += fila;
        });
...
$("section").each(function () {
  ...
  $("#menu-nav").append('<li>' + 
      '<a class="dropdown-item" href="#' + idSeccion + '">' + titulo + '</a>' + 
    '</li>');
